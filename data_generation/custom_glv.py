...
def generate_parameters(N=7, x0_min =0.05, x0_max = 0.3, A_min = -1, A_max =0.2):
    ...
    for i in range(N):
        ...
        # Set diagonal element according to formula: a_ii = -(sum(|a_ij|) + noise)
        A[i, i] = -(row_sum + noise)


    r = np.random.exponential(0.5, N) # exponencial rate 0.5
    K = np.random.uniform(200,1000, N)
